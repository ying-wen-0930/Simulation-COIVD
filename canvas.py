...
class Canvas(object):
    def __init__(self, peoples):
        ...
        plt.style.use('ggplot') # 背景
        self.fig.patch.set_facecolor('white') # 补丁颜色
        # 指定字体
        ...
        grid = plt.GridSpec(4, 16, wspace=0.5, hspace=0.3)

        # 创建子图并按网格布局分布
        self.subplots = [
            plt.subplot(grid[0:4, 0:8]),
            plt.subplot(grid[0:4, 8]),
            plt.subplot(grid[0, 9:15]),
            plt.subplot(grid[1, 9:15]),
            plt.subplot(grid[2, 9:15]),
            plt.subplot(grid[3, 9:15]),
        ]
    ...
    def draw_ax0(self,
        ...
        title = f'''Day of：{time} In New York City (all numbers in thousand)
                    total healthy（grey)：{uninfecated_num};  total cases（red)：{CITY_PEOPLE_NUM-uninfecated_num};  total deaths（black)：{death_num}
                    new Infected：{confirmed_num};  new recovered：{immune_num};  new quarantine：{isolation_num}
    ...
    def draw_ax1(self, ax1, hospital_status, bed_num):
        ...
        title = f'''Bed Usage：{bed_num/BED_NUM}'''
    ...
    def draw_ax5(self, ax5, time, dangerous_people_num):
        ...
        title = 'Lack of Hospital Bed'
        self.draw_line(ax5, time, self.dangerous_data, PEOPLE_COLORS[8], title)
    ...
    def animate(self, time):
        ...
        self.draw_ax5(self.subplots[4], time, dangerous_people_num)
        self.draw_ax4(self.subplots[5], time, death_num)
